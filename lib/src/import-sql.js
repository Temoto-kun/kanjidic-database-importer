(function () {
    ...
    function createColumns(table, obj, parentAttrName) {
        Object.keys(obj)
            .forEach(attrName => {
                ...
                switch (attrType) {
                    case 'integer':
                        table.integer(safeAttrName);
                        break;
                    case 'string':
                        table.string(safeAttrName, attrLength);
                        break;
                    default:
                        break;
                }
                ...
                if (typeof obj[attrName] === 'object') {
                    createColumns(table, obj[attrName], (!!parentAttrName ? parentAttrName + '.' : '') + attrName);
                }
            });
    }
    ...
    function createBaseTable(baseTableName, model) {
        ...
        return db.schema.dropTableIfExists(safeBaseTableName)
            .then(() => db.schema.createTable(safeBaseTableName, (table) => {
                table.increments('id');
                createColumns(table, model.single);
            }));
    }
    ...
    function createJoinTable(baseTableName, entityName, model) {
        ...
        return db.schema.dropTableIfExists(joinTableName)
            .then(() => db.schema.createTable(joinTableName, (table) => {
                table.increments('id');
                table.integer(`${safeBaseTableName}_id`);
    ...
    function createTables(baseTableName, model) {
        var promises = [createBaseTable(baseTableName, model)];

        Object.keys(model.multiple)
            .forEach(entityName => promises.push(
                createJoinTable(baseTableName, entityName, model)
            ));
